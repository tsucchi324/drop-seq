...
<< COMMENT
USAGE: qsub -cwd -l s_vmem=50G,mem_req=50G /home/tissei/drop-seq.sh -f R1_barcode.fastq.gz -r R2_transcript.fastq.gz -i SAMPLE1 -c 500 -m DROP

    Desctiption:
          mapping singlecell data and make umi count data

    Options
        -f ... Input Read1 path (barcode read)
        -r ... Input Read2 path (transcript read)
        -i ... analys ID .This ID use output file names
...
bigwig(){
        ...
        bedGraphToBigWig ${WDIR}/bigwig/bigwig.ucsc.bedGraph $mm10 ${WDIR}/bigwig/bigwig.ucsc.bw
